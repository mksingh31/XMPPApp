            ...
            while (!done && (writerThread == thisThread)) {
                ...
                if (packet != null) {
                    synchronized (writer) {
                        ...
                        Log.d(TAG, "From writePacket: " + packet.toString());
                        writer.flush();
            ...
            try {
                synchronized (writer) {
                   while (!queue.isEmpty()) {
                       ...
                       Log.d(TAG, "From nextWritePacket: " + packet.toString());
                        writer.write(packet.toXML());
                    }
                    writer.flush();
                }
                ...
                Log.d(TAG, "From writePacket Exception: ");
                e.printStackTrace();
            }
    ...
    private class KeepAliveTask implements Runnable {
        ...
        public void run() {
            try {
                ...
                Log.d(TAG, "calling sleeping 15 second plus delay:");
            }
            catch (InterruptedException ie) {
                // Do nothing
            }
            while (!done && keepAliveThread == thread) {
                ...
                try {
                    ...
                    Log.d(TAG, "Sleep until we should write the next keep-alive.");
                    Thread.sleep(delay);
                }
                catch (InterruptedException ie) {
                    // Do nothing
