        ...
        try {
            ...
            do {
                if (eventType == XmlPullParser.START_TAG) {
                    Log.d(TAG, "XmlPullParser.START_TAG");
                    ...
                    // the connectionID lock so that the packet reader startup can finish.
                        ...
                        if ("jabber:client".equals(parser.getNamespace(null))) {
                            Log.d(TAG, "jabber:client");
                            // Get the connection id.
                            for (int i=0; i<parser.getAttributeCount(); i++) {
                                if (parser.getAttributeName(i).equals("id")) {
                                    Log.d(TAG, "AttributeName:id");
                                    // Save the connectionID
                                    connectionID = parser.getAttributeValue(i);
                                    Log.d(TAG, "AttributeName:id : " + connectionID);
                                    if (!"1.0".equals(parser.getAttributeValue("", "version"))) {
                                        Log.d(TAG, "Version is not eqal to 1.0");
                                    ...
                                    String from = parser.getAttributeValue(i);
                                    Log.d(TAG, "from: " + from);
                                    // Use the server name that the server says that it is.
                                    connection.config.setServiceName(from);
                                }
                            }
                        }
                    }
                        ...
                        if ("urn:ietf:params:xml:ns:xmpp-tls".equals(namespace)) {
                            Log.d(TAG, "urn:ietf:params:xml:ns:xmpp-tls:");
                            // TLS negotiation has failed. The server will close the connection
                            throw new Exception("TLS negotiation has failed");
                        }
                        else if ("http://jabber.org/protocol/compress".equals(namespace)) {
                            Log.d(TAG, "http://jabber.org/protocol/compress");
                            ...
                            Log.d(TAG, "SASL authentication has failed");
                        ...
                        Log.d(TAG, "challenge: " + challengeData);
                ...
                Log.d(TAG, "parser.next() start:");
                eventType = parser.next();
                Log.d(TAG, "parser.next(): end");
